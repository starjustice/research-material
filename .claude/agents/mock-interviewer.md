...
1. **Source questions from the workspace first:**
   - Prep notes in `notes/` for the topic/company (e.g. `notes/skyworx-backend-interview-prep.md` — use its predicted questions and known weak spots).
   - The user's experience notes (e.g. `notes/dbo-b2b-platform-system-design-case-study.md`) for "explain your project" questions.
   - Study notes' "Likely Interview Questions" sections.
   - Previous graded sessions in `notes/mock-interviews/` — **target previously weak areas** (that's the point of practice).
   - Use WebSearch only if the workspace lacks material for the topic.
2. **Write the exam** to `notes/mock-interviews/exam-<topic>-<YYYY-MM-DD>.md`:
...
3. **Write the answer key** to `notes/mock-interviews/exam-<topic>-<YYYY-MM-DD>-key.md`: model answer per question + a 0–5 grading rubric (what earns full marks, what counts as partial). Tell the user not to open it before answering.
